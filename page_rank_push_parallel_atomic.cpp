  ...
  for (int iter = 0; iter < MAXiter; iter++) {
    ...
    for (uintV u = start; u <= end; u++) {
      ...
      for (uintE i = 0; i < out_degree; i++) {
		...
		while(!pr_next[v].compare_exchange_weak(temp, temp + (pr_curr[u] / (PageRankType) out_degree) ));
		computations++;
      }
    }
    ...
    for (uintV v = start; v <= end; v++) {
      ...
      //pr_curr[v] = pr_next[v];
      PageRankType temp1 = pr_curr[v];
      while(!pr_curr[v].compare_exchange_weak(temp1, pr_next[v]));
      pr_next[v] = 0.0;
      }
...
void pageRankParallel(Graph &g, int max_iters, int t_threads) {
  ...
  uintV n = g.n_;
   ...
   //std::atomic<PageRankType> *pr_curr_instance;
//std::atomic<PageRankType> *pr_next_instance;
  auto* pr_curr_instance = new std::atomic<PageRankType>[n+1];
  auto* pr_next_instance = new std::atomic<PageRankType>[n+1];

  for (uintV i = 0; i < n; i++) {
    pr_curr_instance[i] = INIT_PAGE_RANK;
    pr_next_instance[i] =  0.0;
  }
  pr_curr=pr_curr_instance;
  pr_next=pr_next_instance;
  ...
  for(int i = 0; i < t_threads; i++){
    ...
    //DetailsArr[i].pr_next = pr_next_instance;
    //DetailsArr[i].pr_curr = pr_curr_instance;
...
/////////////
  ...
  for (uintV u = 0; u < n; u++) {
    sum_of_page_ranks += pr_curr_instance[u];
  }
  std::cout << "Sum of page ranks : " << sum_of_page_ranks << "\n";
  std::cout << "Time taken (in seconds) : " << time_taken << "\n";
  //delete[] pr_curr_instance;
  //delete[] pr_next_instance;
}
